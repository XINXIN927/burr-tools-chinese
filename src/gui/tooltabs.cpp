...
TransformButtons::TransformButtons(int x, int y, int w, int h, int type) : layouter_c(x, y, w, h) {
  ...
  (new LFl_Box("翻转",   1, 1, 1, 1))->labelsize(LABEL_FONT_SIZE);
  (new LFl_Box("微移",  3, 1, 2, 1))->labelsize(LABEL_FONT_SIZE);
  (new LFl_Box("旋转", 6, 1, 2, 1))->labelsize(LABEL_FONT_SIZE);
...
ToolsButtons::ToolsButtons(int x, int y, int w, int h) : layouter_c(x, y, w, h) {
  ...
  (new LFl_Box("约束", 3, 1, 2, 1))->labelsize(LABEL_FONT_SIZE);
  ...
  new LFlatButton_c(8, 3, 1, 1, "填充孔洞", "", cb_ToolsButtons_stub, 40);
  new LFlatButton_c(8, 5, 1, 1, "网格缩放", "", cb_ToolsButtons_stub, 41);
...
SizeButtons::SizeButtons(int x, int y, int w, int h, bool addScale) : layouter_c(x, y, w, h) {
  ...
  (new LFl_Box("网格", 0, 1, 1, 1))->labelsize(LABEL_FONT_SIZE);
  ...
  if (addScale) {
    (new LFl_Box("形状", 2, 1, 1, 1))->labelsize(LABEL_FONT_SIZE);
...
ToolTab_0::ToolTab_0(int x, int y, int w, int h) : ToolTab(x, y, w, h) {

  {
    ...
    toAll = new LFl_Check_Button("应用到所有形状", 0, 0, 1, 1);
    ...
    for (int s = ss; s < se; s++) {
      ...
      switch(task) {
        ...
        case 25:
                 ...
                 break;
        case 26: fl_message("抱歉，菱形网格尚未支持最小化功能！"); return;
        case 40: space->fillHoles(0); break;
