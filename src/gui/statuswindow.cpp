...
class StatusProgress : public LFl_Double_Window {
  ...
  public:
    ...
    StatusProgress(void) : LFl_Double_Window(false) {
      ...
      LFl_Button * btn = new LFl_Button("取消", 0, 2, 1, 1);
      btn->pitch(3);
...
statusWindow_c::statusWindow_c(puzzle_c * p) : LFl_Double_Window(true), puz(p), again(false) {
  ...
  (new LFl_Box("形状", col++, 0))->pitch(2);
  new LFl_Line(col++, 0, 1, lines+head, 2);

  (new LFl_Box("单位", col, 0, 5))->pitch(2);
  (new LFl_Box("普通", col++, 1))->pitch(2);
  new LFl_Line(col++, 1, 1, lines+head-1, 1);
  (new LFl_Box("可变", col++, 1))->pitch(2);
  new LFl_Line(col++, 1, 1, lines+head-1, 1);
  (new LFl_Box("总计", col++, 1))->pitch(2);
  new LFl_Line(col++, 0, 1, lines+head, 2);

  (new LFl_Box("相同", col, 0, 5))->pitch(2);
  (new LFl_Box("镜像", col++, 1))->pitch(2);
  new LFl_Line(col++, 1, 1, lines+head-1, 1);
  (new LFl_Box("形状", col++, 1))->pitch(2);
  new LFl_Line(col++, 1, 1, lines+head-1, 1);
  (new LFl_Box("完全", col++, 1))->pitch(2);
  new LFl_Line(col++, 0, 1, lines+head, 2);

  (new LFl_Box("连通性", col, 0, 5))->pitch(2);
  (new LFl_Box("面", col++, 1))->pitch(2);
  new LFl_Line(col++, 1, 1, lines+head-1, 1);
  (new LFl_Box("边", col++, 1))->pitch(2);
  new LFl_Line(col++, 1, 1, lines+head-1, 1);
  (new LFl_Box("角", col++, 1))->pitch(2);
  new LFl_Line(col++, 0, 1, lines+head, 2);

  (new LFl_Box("孔洞", col, 0, 3))->pitch(2);
  ...
  if (p->getGridType()->getType() == gridType_c::GT_BRICKS) {
    ...
    (new LFl_Box("工具", col, 0, 3))->pitch(2);
    (new LFl_Box("开槽", col++, 1))->pitch(2);
    new LFl_Line(col++, 1, 1, lines+head-1, 1);
    (new LFl_Box("铣削", col++, 1))->pitch(2);
    new LFl_Line(col++, 0, 1, lines+head, 2);
  }

  (new LFl_Box("对称", col++, 0, 1))->pitch(2);
  ...
  LFl_Button * btn = new LFl_Button("关闭", 0, 1);
  btn->callback(cb_Close_stub, this);
  btn->weight(1, 0);

  (new LFl_Box(1, 1))->setMinimumSize(5, 0);

  btn = new LFl_Button("移除选中", 2, 1);
