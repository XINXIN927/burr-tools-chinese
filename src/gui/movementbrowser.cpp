...
AddMovementDialog::AddMovementDialog(movementCache_c * c, const std::vector<unsigned int> & pieces, const problem_c * puz) : LFl_Double_Window(false) {
  ...
  (new LFlatButton_c(0, 0, 1, 1, "完成", "", cb_BtnDone_stub, this))->weight(1, 0);
  (new LFl_Box("", 1, 0, 1, 1))->setMinimumSize(5, 0);
  (new LFlatButton_c(2, 0, 1, 1, "取消", "", cb_BtnCancel_stub, this))->weight(1, 0);
...
movementBrowser_c::movementBrowser_c(problem_c * puzzle, unsigned int solNum) : LFl_Double_Window(true) , puz(puzzle) {
  ...
  analyzeNode =       new LFlatButton_c(0, 0, 1, 1, "分析", " 分析此节点的可能运动 ", cb_NodeAnalyze_stub, this);
  analyzeNextLevels = new LFlatButton_c(1, 0, 1, 1, "深入分析", " 分析多层级的运动 ", cb_NodeAnalyzeMany_stub, this);
  addMovement =       new LFlatButton_c(0, 1, 1, 1, "添加运动", " 向当前节点添加固定运动并查看结果 ", cb_AddMovement_stub, this);
  pruneTree =         new LFlatButton_c(1, 1, 1, 1, "修剪树", " 移除除选中节点路径外的所有节点 ", cb_Prune_stub, this);
  stepBack =            new LFlatButton_c(0, 2, 1, 3, "后退", " 跳转到树中选中节点的父节点 ", cb_StepBack_stub, this);
