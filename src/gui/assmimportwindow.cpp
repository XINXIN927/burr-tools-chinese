...
assmImportWindow_c::assmImportWindow_c(const puzzle_c * puzzle) : LFl_Double_Window(false), _ok(false)
{
  if (puzzle->getNumberOfProblems() == 0)
  {
    ...
    (new LFl_Button("中止", 0, 1, 1, 1))->callback(cb_WindowButton_stub, this);
    return;
  }
  ...
  new LSeparator_c(0, ypos++, 1, 1, "源", false);
  ...
  new LSeparator_c(0, ypos++, 1, 1, "目标", false);

  rdDontAdd = new LFl_Radio_Button("仅将形状添加到拼图", 0, ypos++, 1, 1);
  rdAddNew = new LFl_Radio_Button("将形状添加到新问题", 0, ypos++, 1, 1);
  rdAddDst = new LFl_Radio_Button("将形状添加到已有问题", 0, ypos++, 1, 1);
  rdDontAdd->set();
  ...
  new LSeparator_c(0, ypos++, 1, 1, "范围", false);

  {
    layouter_c * o = new layouter_c(0, ypos++);

    (new LFl_Box("最小范围", 0, 0))->stretchRight();
    (new LFl_Box("最大范围", 0, 1))->stretchRight();
  ...
  new LSeparator_c(0, ypos++, 1, 1, "过滤", false);

  // The filter group
  ckDrpDisconnected = new LFl_Check_Button("丢弃不连续形状", 0, ypos++, 1, 1);
  ckDrpDisconnected->value(1);
  ckDrpMirror = new LFl_Check_Button("丢弃镜像对称形状", 0, ypos++, 1, 1);
  ckDrpSymm = new LFl_Check_Button("丢弃所有对称形状", 0, ypos++, 1, 1);
  if (puzzle->getGridType()->getType() == gridType_c::GT_BRICKS)
  {
    ckDrpMillable = new LFl_Check_Button("丢弃不可铣削形状", 0, ypos++, 1, 1);
    ckDrpNotchable = new LFl_Check_Button("丢弃不可开槽形状", 0, ypos++, 1, 1);
  }
  else
  {
  ...
  ckDrpIdentical = new LFl_Check_Button("移除相同形状", 0, ypos++, 1, 1);
...

  {
    ...
    (new LFl_Box("最小形状", 0, 0))->stretchRight();
    (new LFl_Box("最大形状", 0, 1))->stretchRight();
    (new LFl_Box("", 1, 0))->setMinimumSize(5, 0);
  ...
  (new LFl_Button("继续", 0, 0, 1, 1))->callback(cb_WindowButton_stub2, this);
  (new LFl_Button("中止", 1, 0, 1, 1))->callback(cb_WindowButton_stub, this);
