...
imageExport_c::imageExport_c(puzzle_c * p) : LFl_Double_Window(false), puzzle(p), working(false), state(0), i(0) {
...

  {
    ...
    BgWhite = new LFl_Radio_Button("白色背景", 0, 0);
    BgTransp = new LFl_Radio_Button("透明背景", 0, 1);
    BgWhite->value(1);
...

  {
    ...
    AA1 = new LFl_Radio_Button("无抗锯齿", 0, 0);
...

    {
      ...
      ColPiece = new LFl_Radio_Button("使用部件颜色", 0, 5);
      ColConst = new LFl_Radio_Button("使用颜色约束颜色", 0, 6);
    ...
    DimStatic = new LFl_Check_Button("变暗静态部件", 0, 7);
...

  {
    ...
    SzA4Port = new LFl_Radio_Button("A4 竖排", 0, y++, 5, 1);
    SzA4Port->callback(cb_ImageExportSzUpdate_stub, this);

    SzA4Land = new LFl_Radio_Button("A4 横排", 0, y++, 5, 1);
    SzA4Land->callback(cb_ImageExportSzUpdate_stub, this);

    SzLetterPort = new LFl_Radio_Button("Letter 竖排", 0, y++, 5, 1);
    ...
    (new LFl_Box("宽度", 0, y))->stretchRight();
    ...
    (new LFl_Box("高度", 0, y))->stretchRight();
