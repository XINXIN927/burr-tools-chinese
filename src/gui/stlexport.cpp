...
void stlExport_c::cb_FileChooser(void)
{
  ...
  const char * f = fl_file_chooser("选择要写入的 STL 文件", "*.stl", curFile, 0);
...
stlExport_c::stlExport_c(puzzle_c * p) : LFl_Double_Window(true), puzzle(p) {
...

  {
    ...
    Binary = new LFl_Check_Button("二进制 STL", 0, 2, 3, 1);
...

  {
    ...
    BtnStart = new LFl_Button("导出 STL", 0, 0);
...
void stlExport_c::exportSTL(int shape)
{
  ...
  if (fileExists(name))
  {
    if (fl_choice("文件已存在，是否覆盖？", "取消", "覆盖", 0) == 0)
    {
      return;
    }
  }
