...
configuration_c::configuration_c(void) {
  ...
  CNF_BOOL_D("tooltips",          &i_use_tooltips, "使用工具提示", "true");
  CNF_BOOL_D("lightning",         &i_use_lightning, "3D 视图中使用灯光", "true");
  CNF_BOOL_D("fadeout",           &i_use_blendedRemoving, "淡出部件", "true");
  CNF_BOOL_D("displaylists",      &i_use_displayLists, "使用 openGL 显示列表", "false");
  CNF_BOOL_D("rotator",           &i_rotationMethod, "使用新旋转方法", "true");
...
void configuration_c::dialog(void) {
  ...
  LFl_Button * btn = new LFl_Button("关闭", 0, y, 1, 1);
  btn->tooltip("关闭窗口");
