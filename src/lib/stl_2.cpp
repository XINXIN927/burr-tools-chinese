...
Polyhedron * stlExporter_2_c::getMesh(const voxel_c & v, const faceList_c & /* no holes in this mesh */) const
{
  ...
  if (sphere_rad < Epsilon) throw stlException_c("球体尺寸过小");
  if (offset < 0) throw stlException_c("偏移量不能为负");
  if (round < 0) throw stlException_c("曲率半径不能为负");
  if (offset > sphere_rad) throw stlException_c("偏移量必须小于球体半径");
  ...
  if (inner_rad < 0 || inner_rad >= sphere_rad)
    throw stlException_c("内半径不能为负且必须小于球体半径");
  if (hole_diam < 0 || hole_diam > 0.333334*sphere_rad)
      throw stlException_c("孔径不能为负或大于球体半径的1/3");
