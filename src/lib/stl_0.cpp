...
{
  if (v.countState(voxel_c::VX_VARIABLE)) throw stlException_c("含有可变体素的形状无法导出");
  if (cube_scale < Epsilon) throw stlException_c("单位大小过小");
  if (shrink < 0) throw stlException_c("偏移量不能为负");
  if (bevel < 0) throw stlException_c("倒角不能为负");
  if (tubes > 1) throw stlException_c("管径过大");
  if (!v.meshParamsValid(bevel/cube_scale, shrink/cube_scale)) throw stlException_c("倒角和偏移量无效");
...
const char * stlExporter_0_c::getParameterName(unsigned int idx) const
{
  switch (idx)
  {
    ...
    case 3: return "壁厚";
    case 4: return "管径";
    case 5: return "保留内部凹槽";
    case 6: return "保留外部凹槽";
    case 7: return "移除空洞中的凹槽";
    default: return 0;
  }
}
