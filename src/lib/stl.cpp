...
void stlExporter_c::write(const char * fname, const voxel_c & v, const faceList_c & holes)
{
  ...
  if (binaryMode)
  {
    ...
    if (!f) throw stlException_c("无法打开文件");

    int pos = 0;

    for (int i = 0; i < 84; i++)
    {
      if (fwrite(title+pos, 1, 1, f) != 1) throw stlException_c("无法写入文件");
      if (title[pos]) pos++;
    }
  }
  else
  ...
  try
  {
    ...
    if (!poly) throw stlException_c("生成 STL 多面体时出现问题");
  }
  ...
  for(Polyhedron::const_face_iterator it=poly->fBegin(); it!=poly->fEnd(); it++)
  {
    ...
    do
    {
      ...
      {
        // write normal vector
        if (fwrite(normal, 3, 4, f) != 4) throw stlException_c("无法写入文件");

        // write the 3 vertices
        if (fwrite(v1, 3, 4, f) != 4) throw stlException_c("无法写入文件");
        if (fwrite(v2, 3, 4, f) != 4) throw stlException_c("无法写入文件");
        if (fwrite(v3, 3, 4, f) != 4) throw stlException_c("无法写入文件");

        // attribute
        int i = 0;
        if (fwrite(&i, 1, 2, f) != 2) throw stlException_c("无法写入文件");
  ...
  if (binaryMode)
  {
    ...
    fseek(f, 80, SEEK_SET);
    if (fwrite(&triangleCount, 1, 4, f) != 4) throw stlException_c("无法写入文件");
  }
  else
  {
    fprintf(f, "endsolid\n");
